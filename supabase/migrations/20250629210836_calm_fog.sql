/*
  ...
  1. New Courses
    - Adds courses for all major language pair combinations
    - French, Portuguese, English from Spanish
    - Spanish, French, Italian, Portuguese from English  
    - Spanish, Italian from French
    - Spanish, Italian from Portuguese
    - Spanish, French from Italian

  2. Sample Content
    - Parts, phases, and units for key courses
    - Sample exercises with proper localization
    - Conversation agents configured for each language

  3. Data Integrity
    - Uses UPSERT pattern to avoid conflicts
    - Updates existing courses where appropriate
    - Maintains referential integrity
*/

-- First, let's check what courses already exist and handle conflicts properly
...
-- First, get the actual course ID for French from Spanish
DO $$
DECLARE
