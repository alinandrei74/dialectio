...
DECLARE
    ...
    lessons_is_view boolean;
BEGIN
    -- Verificar si lessons existe como tabla
    SELECT EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_name = 'lessons' AND table_type = 'BASE TABLE'
    ) INTO lessons_exists;
    
    -- Verificar si lessons existe como vista
    SELECT EXISTS (
        SELECT 1 FROM information_schema.views 
        WHERE table_name = 'lessons'
    ) INTO lessons_is_view;
    ...
    IF lessons_exists THEN
        ...
        -- Eliminar FK antigua y crear nueva
        ...
        -- Renombrar columnas en exercises
        ...
        -- Renombrar columnas en user_progress
        ...
        -- Primero eliminar cualquier vista existente
        IF lessons_is_view THEN
            DROP VIEW lessons CASCADE;
        END IF;
        
        -- Renombrar tabla a backup
        ALTER TABLE lessons RENAME TO lessons_backup;
        
        -- Limpiar tabla temporal
        DROP TABLE IF EXISTS _lesson_unit_map;
        
    ELSIF lessons_is_view THEN
        -- Si lessons ya es una vista, simplemente la eliminamos
        DROP VIEW lessons CASCADE;
    END IF;
END $$;
