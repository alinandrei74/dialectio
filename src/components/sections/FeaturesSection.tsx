...
function FeaturesSection({ t }: FeaturesSectionProps) {
  const features = [
    ...
    },
    {
      ...
      bgColor: 'bg-gray-50/90 dark:bg-gray-700/90'
    }
  ];

  return (
    <section id="metodo" className="py-21 bg-white/10 dark:bg-gray-900/20 backdrop-blur-md relative">
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute top-1/4 left-10 w-24 h-2 bg-gradient-to-r from-green-800 to-black dark:from-green-600 dark:to-gray-800 transform rotate-30 opacity-35 shadow-lg"></div>
        <div className="absolute bottom-1/4 right-10 w-2 h-24 bg-black dark:bg-gray-300 transform -rotate-45 opacity-35 shadow-md"></div>
      </div>
          ...
          {features.map((feature, index) => (
            <div key={index} className={`${feature.bgColor} backdrop-blur-md p-8 shadow-xl hover:shadow-2xl transition-all duration-300 border-4 border-black dark:border-gray-300 transform hover:scale-105 ${index % 2 === 0 ? 'rotate-1' : '-rotate-1'}`}
              ...
              <div className="text-center">
                <div className={`bg-gradient-to-r ${feature.color} p-5 border-3 border-black dark:border-gray-300 transform rotate-45 shadow-xl mx-auto mb-6 w-20 h-20 flex items-center justify-center`}>
                  <feature.icon className="w-10 h-10 text-white transform -rotate-45" />
                </div>
                <h3 className="text-xl font-black text-gray-900 dark:text-gray-100 mb-5">
                  {t[feature.title as keyof Translation] as string}
                </h3>
                <p className="text-gray-900 dark:text-gray-200 leading-relaxed font-bold text-base">
